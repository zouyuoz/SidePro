...
# pre-announced parameters
VSCODE, FONT_BBOX_RATIO, IS_WIDTH = True, 1.0, True
...
def clip_limit_adaptive_histogram_equalization(arr):
    ...
    clahe_img = exposure.equalize_adapthist(img, clip_limit=0.02)
    clahe_img = (clahe_img * 255).astype(np.uint8)
    return clahe_img

# above are main HE & CLAHE function:

# below are secondary functions
...
# clahe = clip_limit_adaptive_histogram_equalization(np.array(image, dtype=np.float32))
resized = block_average_downsample(image, char_length)

# dump resized into histogram function
gray_array_HE = histogram_equalization_2d(resized)
gray_array_CLAHE = clip_limit_adaptive_histogram_equalization(resized)

# we get float array, dump into ascii gray scale function
ascii_lines = []
for i, row in enumerate(gray_array_CLAHE):
